...
function moePageSection(name) {
    const container = $("<div></div>");
    const head = $(`<div style="display: flex; align-items: baseline;"><h6 class="text-muted text-uppercase mt-2 mb-3">${name}</h6></div>`);
    const card = $(`<div class="card"><div class="card-body bg-soft-dark" style="display: flex; justify-content: center;"></div></div>`);
...
<span class="ue_right">
  ...
  class="${overrideClassName}" style="${site.moe
...
<span class="ue_right">
  <input type="button"
  value="Test API Key"
  class="${site.ab ? "btn-sub" : "btn"}" style="background: -webkit-gradient(linear,left top,left bottom,color-stop(0,#7eda37),color-stop(100%,#047d00)); color: white;">
  <label style="margin-left: 4px;">
...
function insertDeliciousSettingsUi(providers) {
    ...
    if (site.moe) {
        ...
        s.appendChild(delicious.settings.createCheckbox("trailerInSidebar", "Trailer In Sidebar", `Keeps the trailer small in the sidebar (original position). Makes "Trailer After Synopsis" option irrelevant.`));
    }
...
like with the other option.`));
    ...
    const clearCacheButton = $(`<input type="button" style="margin-left: 20px;" value="Clear Cache" ${site.moe ? `class="btn btn-secondary"` : ""}/>`);
    clearCacheButton.click(() => {
        clearCache();
        alert("Cache cleared!");
    });
    $(s).append(clearCacheButton);
    const resetSettingsButton = $(`<input type="button" style="margin-left: 20px;" value="Reset Settings" ${site.moe ? `class="btn btn-secondary"` : ""}/>`);
    ...
    const resetApiKeysButton = $(`<input type="button" style="margin-left: 20px;" value="Reset API Keys" ${site.moe ? `class="btn btn-secondary"` : ""}/>`);
...
function abInjectLinksToPage(outLinks) {
...
function moeInjectLinksToPage(outLinks) {
    const card = $(`#layout-wrapper div.row > div:first-child > div.card:nth-child(3) > div.card-body`);
    outLinks.forEach((link) => {
        const span = $(`<span class="fw-semibold">${link.name}</span>`);
        const p = $(`
      <p class="font-size-13 mb-2">
        <a href="${link.url}" target="${settings.linksInNewTab ? "_blank" : ""}">${link.url}</a>
      </p>
    `);
        card.append(span);
        card.append(p);
...
;// CONCATENATED MODULE: ./src/helpers/ratingTextColors.ts
...
<div
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
    ...
    // Votes
    const votes = $(`<span style="color: ${siteColors.textSecondary};font-size: 12.5px;"></span>`).appendTo(ratingContainer);
    // Stars
...
<div
  ...
  `);
    ...
    if (site.ab) {
        setTimeout(() => subscribeToAbScoreChange((abScore) => updateRating(abScore)), 1);
    }
    else if (site.moe) {
        updateRating();
    }
...
function abInjectRatingsToPage(scores) {
    if (scores.length === 0) {
        return;
    }
    // General layout
...
function moeInjectRatingsToPage(scores) {
    if (scores.length === 0) {
        return;
    }
    // General layout
    const synopsisTitle = $(`#layout-wrapper div.row > div:last-child > h6:contains("Synopsis")`);
...
;// CONCATENATED MODULE: ./src/helpers/moeGetAnimeMetadata.ts
function moeGetAnimeMetadata(prop) {
    return $(`#layout-wrapper div.row > div:first-child > div.card:nth-child(3) > div.card-body > span:contains("${prop}")`)
        .next()
        .text();
}
...
class TmdbProvider extends MetadataProvider {
    ...
    static async identify(type, name, year) {
        ...
        const entry = tierSort(res.results, [
            ...
            (e) => (type === TmdbMediaType.Tv ? e.name : e.title).toLowerCase() === name.toLowerCase(),
        ])[0];
...
function abTmdbQueryFromPage(process = true) {
    const s = $("#content > div.thin > h2:first-child").text().split(" - ");
    const post = s.pop();
    const type = post.startsWith("TV Series") ||
        post.startsWith("ONA") ||
        post.startsWith("OVA")
        ? TmdbMediaType.Tv
...
function moeTmdbQueryFromPage(process = true) {
    const format = moeGetAnimeMetadata("Format");
    log("Moe format found:", format);
    const type = format === "TV" || format === "ONA" || format === "OVA"
        ? TmdbMediaType.Tv
        : format === "Movie"
            ? TmdbMediaType.Movie
            : false;
...
async function sortTrailers(tr) {
...
so it might not be playable.`, err);
    ...
    if (site.ab) {
        ...
        const h6 = $(`#layout-wrapper div.row > div:first-child > h6:contains("Trailer")`);
    ...
    if (trailers.length > 1) {
        // trailers selection
        const select = $(`<select name="trailers" id="${UNIQUE}-trailer-selection" style="margin-left: 10px; padding: 2px; max-width: 90%;${site.moe
            ? `height: 25px;
             border-radius: 5px;
...
;// CONCATENATED MODULE: ./src/helpers/fetchJikans.ts


async function fetchJikans(url) {
    log(`Fetching ${url}`);
    const res = await gmFetchJson({
        method: "GET",
        url: url,
...
;// CONCATENATED MODULE: ./src/helpers/fetchMalExternals.ts



// Map to dedupe duplicate requests by cache key
const pendingRequests = new Map();
async function fetchMalExternals(malId) {
    ...
    const promise = (async () => {
        try {
            const data = (await fetchJikans(`https://api.jikan.moe/v4/anime/${malId}/external`));
...
const flags = site.ab
    ? new Set([ProviderFlags.Score])
    : new Set([ProviderFlags.Score, ProviderFlags.Link]);
...
;// CONCATENATED MODULE: ./src/providers/AnnProvider.ts




class AnnProvider extends MetadataProvider {
    name = "ANN";
...
class MalJikanProvider extends MetadataProvider {
    ...
    async getScore() {
        ...
        const data = await fetchJikans(`https://api.jikan.moe/v4/anime/${this.malId}`);
    ...
    async getTrailers() {
        ...
        const data = await fetchJikans(`https://api.jikan.moe/v4/anime/${this.malId}/videos`);
...
;// CONCATENATED MODULE: ./src/providers/MoeTrailerProvider.ts


class MoeTrailerProvider extends MetadataProvider {
    name = "Moe Trailer";
...
;// CONCATENATED MODULE: ./src/providers/WikipediaProvider.ts




class WikipediaProvider extends MetadataProvider {
    name = "Wikipedia";
    flags = new Set([ProviderFlags.Link]);
    url = false;
    async init() {
        ...
        const externals = await fetchMalExternals(malId);
        const url = externals.find((e) => e.name === "Wikipedia" && e.url.includes("en.wikipedia.org"))?.url;
        if (!url) {
            logError("No wiki external found in MAL externals", externals);
            return false;
        }
        this.url = url;
        return true;
...
async function fetchTrailersFromProviders(providers) {
    const trailers = [];
...
async function fetchLinksFromProviders(providers) {
...
async function fetchRatingsFromProviders(providers) {
...
async function src_main() {
    ...
    // Providers
    const siteSpecificProviders = site.ab
        ? []
    ...
    fetchLinksFromProviders(providers.filter((p) => p.isEnabled() && p.flagEnabled(ProviderFlags.Link))).then((links) => {
        if (site.ab) {
    ...
    fetchRatingsFromProviders(providers.filter((p) => p.isEnabled() && p.flagEnabled(ProviderFlags.Score))).then((ratings) => {
        if (site.ab) {
    ...
    fetchTrailersFromProviders(providers.filter((p) => p.isEnabled() && p.flagEnabled(ProviderFlags.Trailers))).then((trailers) => {
